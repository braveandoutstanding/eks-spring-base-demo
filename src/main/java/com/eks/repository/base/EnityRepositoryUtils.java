...
public interface EnityRepositoryUtils<T,ID extends Serializable> extends JpaRepository<T,ID>, JpaSpecificationExecutor<T>{
    ...
    List<T> findAllByCreateDateAndUpdateDateAndRecordStatus(Date createDate,Date updateDate,Integer recordStatus);//确保T必须有createDate、updateDate、recordStatus字段
}